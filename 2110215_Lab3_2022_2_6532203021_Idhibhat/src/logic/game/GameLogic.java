...
public class GameLogic {
    ...
    public boolean isHandPlayable(int playerIndex) {
        // TODO Implement here
    	boolean flag =false;
    	for(int i=0;i< playerHands.get(playerIndex).size();i++) {
    		BaseCard card = playerHands.get(playerIndex).get(i);
    		if(card.canPlay()) {
    			flag = true;
    			break;
    		}
    ...
    public ArrayList<BaseCard> draw(int amount) {
        // TODO Implement here
    	int amountToDraw = Math.min(amount, GameLogic.getInstance().getDeck().size());
    	ArrayList<BaseCard> drawDeck = new ArrayList<>();
    	for(int i=0;i<amountToDraw;i++) {
    		drawDeck.add(GameLogic.getInstance().getDeck().remove(0));
    	}
    	GameLogic.getInstance().getCurrentPlayerHand().addAll(drawDeck);
    	return drawDeck;
    }
